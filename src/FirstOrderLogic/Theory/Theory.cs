...
namespace FirstOrderLogic
{
    public class Theory : ITheory
    {
        ...
        // Set semantics: sentences already present (by Equals) are not added again. The set only
        ...
        public bool IsConsistentWith(ITheory? other, ComparisonMode mode = ComparisonMode.Syntactic, int maxRounds = 0)
        {
            switch (mode)
            {
                ...
                case ComparisonMode.Semantic:
                    // Pass the raw conjunction: refuting it only needs equisatisfiability, so
                    // IsUnsatisfiable may skolemize — which ToConjunctiveNormalForm, an
                    // equivalence, refuses to do on our behalf for existentials.
                    var union = Union(other);
                    return union.Count == 0 || !Resolution.IsUnsatisfiable(Conjoin(union), maxRounds: maxRounds);
                default:
        ...
        public bool Entails(ISentence target, int maxRounds = 0)
        {
            // The empty theory entails exactly the tautologies: ⊨ target iff ¬target is unsatisfiable.
            if (this.Count == 0)
            {
                return Resolution.IsUnsatisfiable(target.Negated(), maxRounds: maxRounds);
            }

            return Resolution.Resolve(Conjoin(this), target, maxRounds: maxRounds);
        }

        public List<List<ISentence>> Explain(ISentence target, int maxRounds = KernelSets.DefaultMaxRounds) =>
            new KernelSets(maxRounds).FindAllKernels(this, target);

        // A null comparand is the empty theory, as it is for IsConsistentWith: it holds and denies
        // nothing, so every sentence here comes back a silence.
        public Stance Compare(ITheory? other, ComparisonMode mode = ComparisonMode.Syntactic, int maxRounds = 0)
        {
            other ??= new Theory(new List<ISentence>());
            ...
            {
                if (HeldByOther(other, s, closure, maxRounds))
                {
                    agree.Add(s);
                }
                else if (DeniedByOther(other, s, closure, maxRounds))
                {
                    disagree.Add(s);
                }
                else
        ...
        private static bool HeldByOther(ITheory other, ISentence s, IReadOnlyList<ISentence>? closure, int maxRounds)
        {
            if (closure == null)
            {
                return other.Entails(s, maxRounds);
            }
        ...
        private static bool DeniedByOther(ITheory other, ISentence s, IReadOnlyList<ISentence>? closure, int maxRounds)
        {
            if (HeldByOther(other, s.Negated(), closure, maxRounds))
            {
                return true;
            }
        ...
        {
            if (rule.Premises.Count != candidate.Premises.Count)
            {
                return false;
            }

            if (rule.NafPremises.Count != candidate.NafPremises.Count)
            {
                return false;
            }
            ...
            bool MatchFrom(List<ISentence> left, int i, List<ISentence> right, bool[] used)
            {
                ...
                for (var j = first; j <= last; j++)
                {
                    ...
                    if (!Unificator.TryMatch(left[i], right[j], out var match))
                    {
                        continue;
                    }
                    ...
                    used[j] = true;
                    if (MatchFrom(nextLeft, i + 1, nextRight, used))
                    {
                        return true;
                    }
